...
- **데스크톱 위젯**: 시스템 개요, AI 구독 한도, 명절·황력 달력, 내일 근무 여부, IP 신뢰도, 공인 IP. AI·IP·달력 데이터는 메인 앱의 로컬 캐시에서만 읽습니다.
...
시스템 모니터링 데이터는 Mac에 남으며 XStats 계정은 필요하지 않습니다. AI 사용량 및 한도는 기본적으로 꺼져 있습니다. 켜면 로컬 통계는 세션 로그를 읽고, 구독 한도 조회는 해당 CLI의 로그인 정보를 변경하지 않고 읽어 각 제공자에게 직접 요청합니다. 자신의 Sub2API 서버를 예비 한도 소스로 설정할 수 있습니다.
