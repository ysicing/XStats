...
[![Release](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-0.8.1-6ee02b)](https://github.com/ysicing/xstats/releases)
...
网速、磁盘、电池、温度和风扇，也可以按需查看 Codex / Claude Code 本机模型使用统计；并且可以直接处理：给风扇提速、合盖后继续运行、清理缓存、彻底卸载应用、停用启动项；
网络详情还能检测公网 IP 的纯净度，看出出口是否被标记为 VPN、代理、机房或有滥用记录。
所有系统监控指标都在你自己的 Mac 上读取，不会上传。不需要 XStats 账号，可自行配置 WebDAV 服务器，手动备份和恢复偏好设置；XStats 不提供或预设同步服务器。检查更新时会发送当前版本和随机安装标识的 SHA-256，用于去重安装与版本分布统计；原始随机值只保存在本机偏好设置中，不作为凭据，也不参与 WebDAV 同步。其他可关闭的联网功能包括：打开网络详情时查询公网 IP 与纯净度、定时 ping 你选择的探测目标、每天检查一次新版本。
...
历法由 [Tyme4Swift 1.5.0](https://github.com/6tail/tyme4swift) 在本机计算，采用 MIT 许可，
见[第三方声明](ThirdPartyNotices.md)。月历可浏览 1900–2100 年；藏历数据覆盖公历 1951-01-08 至 2051-02-11，
中国法定假日与调休数据当前收录到 2026 年，超出覆盖范围会提示。梅雨天是传统历法推算，并非天气预报。
...
- 菜单栏日历和“AI 用量与额度”改为独立可选模块；关闭后隐藏对应入口。日历统一在设置中启用，启用后通过设置按钮打开独立弹窗配置，不再重复显示开关。
...
- 新增 AI 用量与额度：只读本机 Codex / Claude Code 会话日志，按模型展示 Token、缓存命中率、用量记录、每日趋势；默认显示近一年活动热力图，提供每日、每周、累计三种全年活动视图及模型筛选；SQLite 持久化扫描游标与统计，追加日志只读增量，菜单栏显示今日 Token。
...
- **AI 用量与额度**：只读本机 Codex / Claude Code 会话日志，按模型统计输入、输出、缓存 Token；默认显示近一年活动热力图，提供每日、每周、累计三种全年活动视图及模型筛选；SQLite 保存扫描游标和统计结果，重启可复用，日志追加时只读新增部分。启用后还会自动使用本机 CLI 登录凭据直接查询两家服务的 5 小时及每周额度。
两个来源都启用且至少一个有额度时，菜单栏 AI 项会分别显示 Codex 和 Claude 的剩余比例；没有额度的来源显示占位，悬停提示列出重置时间。
...
**用 Apple 智能解释进程**——在支持的 macOS 上右键选择「用 Apple 智能解释」，了解进程是什么、占用是否正常、退出有什么影响。仅使用本机模型；模型不可用时显示原因，不会调用其他服务。回答可能不准确，结束进程前请自行确认。
...
详细说明见[隐私政策](Packages/XStatsKit/Sources/XStatsUI/Resources/Legal/Privacy.zh-Hans.md)和[服务条款](Packages/XStatsKit/Sources/XStatsUI/Resources/Legal/Terms.zh-Hans.md)，也可在应用的“设置 → 关于”中阅读。

指标来自本机的内核接口（`host_processor_info`、`host_statistics64`、`sysctl`）、IOKit 与 SMC。
偏好设置保存在应用自己的 user defaults 中。除更新统计汇总及你主动上传的 WebDAV 设置备份外，应用自身的其他持久化数据留在本机；联网工具仍会向相应第三方发送完成查询或测试所需的请求。监控指标、历史记录、硬件序列号和进程列表不会上传。

会联网的功能都可以关闭。公网 IP 与连接探测在网络页中，检查更新在“设置 · 关于”中：
...
**AI 用量与额度**默认关闭。启用后，本机 Token 统计只读取 Codex / Claude Code 会话日志；额度部分会只读检查 CLI 登录凭据，直接向 `chatgpt.com` / `api.anthropic.com` 查询 5 小时及每周用量。登录令牌只发送到对应服务，不保存到 XStats 设置或统计数据库，日志内容不随请求发送。额度接口可能变化或限流，失败时不影响本机统计。
每个来源最后一次成功的额度会保存在本机 SQLite 数据库中；重启后先显示这份数据并标明获取时间，定时刷新或点击刷新成功后再替换。认证失效或移除手动配置时会清除对应缓存。

若无法通过本机 Codex 或 Claude Code 登录获取额度，可在“AI 用量与额度”设置中分别为这两个来源配置自己的 Sub2API HTTPS 地址、管理员邮箱与密码、账号 ID。每个来源仅在自身自动查询失败时使用对应的备选配置，并核对账号平台；后台请求使用 `force=false` 读取额度，不触发主动探测。可显示 5 小时、7 天，以及 Claude 账号提供时的 Sonnet / Fable 周窗口。两份管理员密码分别保存在本机钥匙串，连接信息只留在本机设置中，不进入 WebDAV 备份。登录请求会把管理员邮箱和密码发送到你配置的 Sub2API 服务器，额度数据不会上传到 XStats 服务器。
Sub2API 未提供 Fable 数值时，页面显示“暂无额度数据”，不会把缺失值当成 0% 已用。

WebDAV 仅在你自行配置服务器并手动同步时传输偏好设置，不上传监控数据、历史记录或 WebDAV 凭据。
...
Apple 智能进程解释在本机完成，不会把进程信息发送给其他 AI 服务。
