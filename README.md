...
- **AI 使用统计**：只读本机 Codex / Claude Code 会话日志，按模型统计输入、输出、缓存 Token；默认显示近一年活动热力图，提供每日、每周、累计三种全年活动视图及模型筛选；SQLite 保存扫描游标和统计结果，重启可复用，日志追加时只读新增部分。启用后还会自动使用本机 CLI 登录凭据直接查询两家服务的 5 小时及每周额度。
...
**AI 使用统计**默认关闭。启用后，本机 Token 统计只读取 Codex / Claude Code 会话日志；额度部分会只读检查 CLI 登录凭据，直接向 `chatgpt.com` / `api.anthropic.com` 查询 5 小时及每周用量。登录令牌只发送到对应服务，不保存到 XStats 设置或统计数据库，日志内容不随请求发送。额度接口可能变化或限流，失败时不影响本机统计。

若无法通过本机 Codex 或 Claude Code 登录获取额度，可在 AI 使用统计设置中分别为这两个来源配置自己的 Sub2API HTTPS 地址、管理员邮箱与密码、账号 ID。每个来源仅在自身自动查询失败时使用对应的备选配置，并核对账号平台；后台请求使用 `force=false` 读取额度，不触发主动探测。可显示 5 小时、7 天，以及 Claude 账号提供时的 Sonnet / Fable 周窗口。两份管理员密码分别保存在本机钥匙串，连接信息只留在本机设置中，不进入 WebDAV 备份。登录请求会把管理员邮箱和密码发送到你配置的 Sub2API 服务器，额度数据不会上传到 XStats 服务器。
