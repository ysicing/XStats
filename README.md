...
- **可选模块**：独立菜单栏日历；本机番茄钟与多屏护眼休息幕布（可随时跳过、暂停或收起到 Mini HUD）；Codex / Claude Code 本机会话 Token 统计和订阅额度查询。
- **桌面小组件**：系统概览、番茄钟、AI 额度、节日黄历日历、整月日历、明天上班吗、IP 纯净度和公网 IP；AI、IP 与日历数据只读取主应用的本地缓存。
