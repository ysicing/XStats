...
所有系统监控指标都在你自己的 Mac 上读取，不会上传。不需要 XStats 账号，可通过自己的 WebDAV 服务器手动备份和恢复偏好设置。检查更新时会发送当前版本和随机安装标识的 SHA-256，用于匿名安装与版本分布统计；原始随机值只保存在本机偏好设置中，不作为凭据，也不参与 WebDAV 同步。其他可关闭的联网功能包括：打开网络详情时查询公网 IP 与纯净度、定时 ping 你选择的探测目标、每天检查一次新版本。
...
**用 AI 解释进程**——右键「用 AI 解释」，说明进程是什么、占用是否正常、退出有什么影响。
「设置 → AI 助手」可选择 Apple 智能、Codex CLI 或 Claude Code；默认使用 Apple 本机模型，
备用 CLI 默认不选；选择 Codex 或 Claude 作为默认或备用服务即表示同意由该 CLI 处理脱敏摘要，
但只有主动点击「解释进程」时才会发起请求。Apple 因设备、系统或模型不可用时，首次提示选择
Codex、Claude 或不使用备用；选择会记住，以后直接使用或显示原错误，也可随时在设置中更改。
CLI 可能联网并消耗额度；只发送脱敏进程摘要，不允许模型执行 Shell、文件操作或 MCP 工具。
CLI 路径可自动发现或手动指定，模型可选填；会话使用临时目录且不写入 CLI 会话历史。
回答可能不准确，结束进程前请自行确认。
...
Apple 智能解释在本机完成；选择并授权 CLI 服务后，进程名称、签名和资源占用摘要会发送给其模型服务。
不发送完整用户路径、命令行参数、环境变量或日志正文。关闭 AI 助手或切换服务会取消当前解释。
