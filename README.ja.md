...
[![Release](https://img.shields.io/badge/version-0.7.1-6ee02b)](https://github.com/ysicing/xstats/releases)
...
設定の AI 助手で Apple Intelligence、Codex CLI、Claude Code を選択できます。Apple は端末内で処理します。
Apple Intelligence が使えない場合、初回に Codex、Claude または予備なしを選択できます。選択は保存され、設定からいつでも変更できます。
Codex または Claude を既定または予備に選ぶと、説明を実行した時に機密情報を省いた概要を CLI に送ることを許可します。選択しただけでは通信しません。
完全なユーザーパスや環境変数は送信せず、モデルによるコマンド実行を無効にします。終了前に説明を確認してください。
...
AI 使用統計は初期状態で無効です。ローカル Codex / Claude Code ログからモデル別 Token、キャッシュヒット率、日別推移を集計します。既定は過去1年の活動ヒートマップ。日別・週別・累計の3表示はいずれも過去1年を表示します。SQLite に読取位置と統計を保存し、再起動後も差分を読み取ります。認証情報や契約上限は読み取りません。
